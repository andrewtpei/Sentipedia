...
```{r}
library(httr);library(jsonlite);library(RedditExtractoR);library(tidyverse);library(purrr);library(tidytext); library(wordcloud2); library(textdata); library(ggraph); library(syuzhet); library(igraph); library(remotes); library(patchwork)
```
...
bicloud1 <- bigrams_separated1 %>% count(bigram, sort = T) %>% filter(n > 5); bicloud2<- bigrams_separated2 %>% count(bigram, sort = T) %>% filter(n > 5); bicloud3 <- bigrams_separated3 %>% count(bigram, sort = T) %>% filter(n > 5); bicloud4 <- bigrams_separated4 %>% count(bigram, sort = T) %>% filter(n > 5) #Converting the 4 split bigrams into a wordcloudable format. 
