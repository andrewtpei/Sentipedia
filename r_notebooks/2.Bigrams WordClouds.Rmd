...
```{r}
library(RedditExtractoR);library(tidyverse);library(purrr);library(tidytext); library(wordcloud2); library(textdata); library(syuzhet); library(gridExtra); library(gridGraphics); library(ggplotify); library(plotly); library(ggwordcloud)
```


```{r}
df_bigrams <- df_Brexit %>%
...
wordcloud2(bicloud1, shape = "circle", fontFamily = "mono")
layout(matrix(c(1, 2), nrow=2), heights=c(1, 4))
par(mar=rep(0, 4))
plot.new()
text(x = 0.5, y = 0.5, "2016-2017", cex = 1.5, font = 2)
