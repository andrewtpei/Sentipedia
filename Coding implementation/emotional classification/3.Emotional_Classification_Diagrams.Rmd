```{r}
library(RedditExtractoR);library(tidyverse);library(tidytext); library(wordcloud2); library(textdata); library(lubridate); library(ggrepel); library(directlabels); library(ggthemes); library(sentimentr); library(gridExtra); library(plotly); library(scales)
```
...
```{r}
articleemotion1 <- Atop_Brexit_urls %>% sentimentr::get_sentences() %>% sentimentr::emotion() %>% rename("Reddit Link" = url) %>% arrange(date_utc)
articleemotion2 <- left_join(df_news, articleemotion1, by = "Reddit Link")
articleemotion2 <- articleemotion1 %>% arrange(timestamp) %>% group_by(url, emotion_type) %>% 
  summarise(Emotion_Percentage = sum(emotion_count)) %>% #group by url link (i.e each news article/post) 
  ...
  mutate(across(-c("Date","rowSum","url","Year"), ~ (./rowSum*100))) %>% #convert into percentages 
  group_by(Year) %>% 
  summarise(across(everything(), mean, na.rm = TRUE)) #warning message is expected, it attempted to summarise the non-numeric/factor urls which do not form part of the analysis. 
```

```{r}
emotionrenamed <- rename(emotion1, "Reddit Link" = url)
emotion4 <- left_join(df_news,)
emotion3 <- emotion1 %>% dplyr::filter(emotion_count > 0) %>% arrange(date) %>% #only keep instances where an emotion count was recorded for a comment
  group_by(url, emotion_type) %>% #group by url link (i.e each news article) and emotions 
  summarise(Emotion_Percentage = sum(emotion_count)) %>% 
  pivot_wider(names_from = emotion_type, values_from = Emotion_Percentage) %>% #rows correspond to each post and columns are counts of each emotion. 
  mutate(across(everything(), ~ replace_na(., 0))) %>% data.frame() %>% #removes all NA values
  ...
  group_by(Year) %>% 
  summarise(across(everything(), mean, na.rm = TRUE)) %>% #warning message is expected, it attempted to summarise the non-numeric/factor urls which do not form part of the analysis. 
 mutate(across(-c("Date","rowSum","url","Year"), ~ (./rowSum*100)))
